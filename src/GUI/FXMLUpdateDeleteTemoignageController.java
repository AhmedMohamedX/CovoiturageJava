 ...
 * @author adel
 */
public class FXMLUpdateDeleteTemoignageController implements Initializable {
    ...
    temoignage tm = new temoignage(Integer.parseInt(idtem.getText()), message.getText());
    temservices.update(tm);
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
     ...
     List<temoignage> list2 =list1.stream().filter(x->x.getUser().getId()==User.getIdofconnecteduser()).collect(Collectors.toList());
     
     ObservableList<temoignage> listtem = FXCollections.observableArrayList(list2);
     ...
     tableview.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection)->{
      ...
      allitems = tableview.getItems();
       selecteditem = tableview.getSelectionModel().getSelectedItems();
        ...
        message.setText(selecteditem.get(0).getMessage());
